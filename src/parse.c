    ...
    *prog = da_new(op_t);
            ...
            if (da_has(&token_builtins, tok.name)) {
                ...
                if (da_has(&bindings_list, tok.name)) {
                    ...
                    if (strlen(tok.name) > MAX_IDENTIFIER_LENGTH) {
                        fprintf(stderr, "%s:%zu:%zu: error: identifier `%s` exceeds maximum length of %d.\n", tok.location.file, tok.location.row, tok.location.col, tok.name, MAX_IDENTIFIER_LENGTH);
                        lexer_free(&lex);
                        exit(1);
                    }
                    op->type = OP_IDENT;
    ...
    if (bind_reference_stack.length != 0) {
        while (bind_reference_stack.length > 0) {
            ...
            fprintf(stderr, "%s:%zu:%zu: error: undefined symbol `%s`.\n", op->location.file, op->location.row, op->location.col, op->name);
        }
        lexer_free(&lex);
        exit(1);
    }
